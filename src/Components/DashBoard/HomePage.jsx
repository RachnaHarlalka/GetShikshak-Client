...
function HomePage({ fetchedData }) {
  ...
  const handleRequestStatus=async(status)=>{
    ...
    try{
      const response = await axios({
        url:"http://localhost:3000/tutor/updaterequeststatus",
        method:"PATCH",
        data:{
      ...
      console.log(response.data.message,response.data.updatedReservationRequest);
      enqueueSnackbar(response.data.message,{variant:"success"});
      closeNotificationDetailsPage();
      console.log("class request",classRequests);
      const updatedClassReuest=classRequests.filter((classRequest)=>{
        return(
          currentNotification._id!==classRequest._id
      )
      })
      setClassRequests(updatedClassReuest);
      console.log("updatedClassReuest",updatedClassReuest);
  ...
  function notificationDetailsPage() {
    return (
      <div
        id="notification-details-div"
        className="sub-container-div"
        ...
        <div className="row-div margin-buttom-div">
          <div className="display-type-flex width-100" id="request-details-div">
            <div className="" id="student-profile-div">
              <img className="student-profilePic" src={`http://localhost:3000/assets/${currentNotification?.studentId?.profilePic}`} alt="" />
            </div>
        ...
        <div className="row-div margin-buttom-div" id="class-options">
          <div className="display-type-flex">
            <div className="sub-container-div" id="requested-subjects-div">
              <div className="div-heading">SUBJECTS</div>
              <div
                className="content-div display-type-flex"
                id="requested-subjects"
              >
                {currentNotification?.subjects.map((subject) => {
                  return (
                    <span className="requested-subject-span">{subject}</span>
                  );
                })}
              </div>
            </div>
            <div className="sub-container-div" id="requested-mode-div">
              <div className="div-heading">MODE OF LEARNING</div>
              <div className="content-div" id="requested-modes">
                {currentNotification?.mode.map((modeType) => {
                  return (
                    <span className="requested-subject-span">{modeType}</span>
                  );
                })}
              </div>
            </div>
        ...
        <div className="row-div margin-buttom-div">
          <div className="width-100 display-type-flex" id="accept-reject-div">
            <div className="content-div">
              <button className="accept-reject-button" id="accept-button" onClick={()=>{handleRequestStatus("accepted")}}>
                ACCEPT
              </button>
              <button className="accept-reject-button" id="reject-button" onClick={()=>{handleRequestStatus("rejected")}}>
                REJECT
              </button>
  ...
  return (
    <div id="home-page-root-div">
      <div id="home-page-details-div">
          ...
          <div id="home-page-top-sub-div">
            <div className="sub-container-div" id="rating-div">
              <div id="rating-outer-div">
                <div id="rating-label-div">Class Rating</div>
        ...
        <div className="row-div" id="last-row">
            ...
            <div className="content-div" id="rate-display-div">
              ...
              <span style={{ fontSize: "large" }}>
                <span style={{ fontWeight: "bold", marginRight: "5px" }}>
                  {userData ? userData.tutorForm.rate : ""}
                </span>
                ₹/hr
              </span>
            </div>
